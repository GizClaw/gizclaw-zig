...
pub fn runSinglePeerTransfer(
    ...
    const initiator_pair = giznet.KeyPair.seed(grt, 901);
    const responder_pair = giznet.KeyPair.seed(grt, 902);
...
pub fn runMultiPeerBidirectionalRekey(
    ...
    const left_hub_pair = giznet.KeyPair.seed(grt, 3001);
    const right_hub_pair = giznet.KeyPair.seed(grt, 3002);
    ...
    for (0..peer_count) |index| {
        left_leaf_pairs[index] = giznet.KeyPair.seed(grt, @intCast(3100 + index));
        right_leaf_pairs[index] = giznet.KeyPair.seed(grt, @intCast(3200 + index));
