    ...
    const Cases = struct {
        ...
        fn runCase(comptime cipher_kind: Cipher.Kind) !void {
            ...
            const label = switch (cipher_kind) {
                .chacha_poly => "giznoise.noise.engine.handshake_transfer_rekey.chacha_poly",
                .aes_256_gcm => "giznoise.noise.engine.handshake_transfer_rekey.aes_256_gcm",
                .plaintext => "giznoise.noise.engine.handshake_transfer_rekey.plaintext",
            };
            ...
            grt.std.debug.print(
                "bench label=giznoise.noise.engine.handshake_transfer_rekey cipher={s} established_events={d} received_packets={d} received_bytes={d} payload_B/s={d} payload_Mbps={d}\n",
                .{
                    @tagName(cipher_kind),
