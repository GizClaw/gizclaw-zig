...
const KeyPair = @import("giznet").KeyPair;
...
pub fn TestRunner(comptime grt: type) glib.testing.TestRunner {
    ...
    const Cases = struct {
        ...
        fn runOfflineDeadlineFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2901);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2902);
        ...
        fn runPassiveKeepaliveFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2911);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2912);
        ...
        fn runInitiateHandshakeKeepaliveConfigFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2916);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2917);
        ...
        fn runDuplicateInitiateHandshakeIsIdempotentFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 3916);
            const remote_pair = giznet.KeyPair.seed(any_lib, 3917);
        ...
        fn runPeerPolicyRejectFlow() !void {
            ...
            const initiator_pair = giznet.KeyPair.seed(any_lib, 2918);
            const responder_pair = giznet.KeyPair.seed(any_lib, 2919);
            const allowed_pair = giznet.KeyPair.seed(any_lib, 2920);
        ...
        fn runPersistentKeepaliveFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2921);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2922);
        ...
        fn runNegativeDurationNormalizationFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2926);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2927);
        ...
        fn runOutboundCallbackErrorOwnershipFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2931);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2932);
        ...
        fn runTransportCallbackErrorOwnershipFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2933);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2934);
        ...
        fn runEmptyTransportSuccessOwnershipFlow() !void {
            ...
            const local_pair = giznet.KeyPair.seed(any_lib, 2935);
            const remote_pair = giznet.KeyPair.seed(any_lib, 2936);
        ...
        fn runRekeyAfterMessagesFlow(comptime cipher_kind: Cipher.Kind) !void {
            ...
            const initiator_pair = giznet.KeyPair.seed(any_lib, 1901);
            const responder_pair = giznet.KeyPair.seed(any_lib, 1902);
        ...
        fn makeSession(
            ...
            return Session.init(.{
                ...
                .peer_key = peer_key,
                .endpoint = endpoint,
                .send_key = giznet.Key{ .bytes = [_]u8{send_fill} ** 32 },
                .recv_key = giznet.Key{ .bytes = [_]u8{recv_fill} ** 32 },
                .timeout = timeout,
                .now = now,
            });
        }
