    ...
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: glib.std.mem.Allocator) bool {
            ...
            tryCase(grt) catch |err| {
                t.logErrorf("giznoise/noise Session unit failed: {}", .{err});
                return false;
            };
            return true;
        }
        ...
        fn tryCase(comptime any_lib: type) !void {
            ...
            const send_key = giznet.Key{ .bytes = [_]u8{0x11} ** 32 };
            const recv_key = giznet.Key{ .bytes = [_]u8{0x22} ** 32 };
            var sender = Session.init(.{
                .local_index = 1,
                .remote_index = 2,
                .peer_key = giznet.Key{ .bytes = [_]u8{0x33} ** 32 },
            ...
            var receiver = Session.init(.{
                .local_index = 2,
                .remote_index = 1,
                .peer_key = giznet.Key{ .bytes = [_]u8{0x44} ** 32 },
