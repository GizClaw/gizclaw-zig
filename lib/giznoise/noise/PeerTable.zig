    ...
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: glib.std.mem.Allocator) bool {
            ...
            tryCase(grt) catch |err| {
                t.logErrorf("giznoise/noise PeerTable unit failed: {}", .{err});
                return false;
            };
            return true;
        }
        ...
        fn tryCase(comptime any_lib: type) !void {
            ...
            const key_a = giznet.KeyPair.seed(any_lib, 51);
            const key_b = giznet.KeyPair.seed(any_lib, 61);
            const key_c = giznet.KeyPair.seed(any_lib, 71);
            const listener_pair = giznet.KeyPair.seed(any_lib, 81);
