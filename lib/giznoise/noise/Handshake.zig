    ...
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: glib.std.mem.Allocator) bool {
            ...
            tryCase(grt) catch |err| {
                t.logErrorf("giznoise/noise Handshake unit failed: {}", .{err});
                return false;
            };
            return true;
        }
        ...
        fn tryCase(comptime any_lib: type) !void {
            ...
            const initiator_pair = giznet.KeyPair.seed(any_lib, 3);
            const responder_pair = giznet.KeyPair.seed(any_lib, 9);
