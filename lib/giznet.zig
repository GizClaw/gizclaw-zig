...
pub const test_runner = struct {
    ...
    pub const integration = struct {
        pub fn make(comptime std: type) embed.testing.TestRunner {
            return @import("giznet/test_runner/integration.zig").make(std);
        }
    };
    pub const benchmark = struct {
        pub fn make(comptime std: type) embed.testing.TestRunner {
            return @import("giznet/test_runner/benchmark.zig").make(std);
        }
    };
...
pub fn make(comptime std: type) type {
    return struct {
        ...
        pub const Key = noise_ns.Key;
        pub const KeyPair = noise_ns.KeyPair;
        pub const noise = noise_ns.make(std, noise_ns.default_cipher_kind);
    };
}
