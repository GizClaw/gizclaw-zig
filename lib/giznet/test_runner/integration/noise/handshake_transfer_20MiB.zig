...
pub fn make(comptime grt: type) testing_api.TestRunner {
    const Cases = struct {
        fn chacha(_: *testing_api.T, allocator: grt.std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_test_utils.runSinglePeerTransfer(grt, .chacha_poly, 20 * 1024 * 1024, grt.std.math.maxInt(u64));
        }

        fn aes(_: *testing_api.T, allocator: grt.std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_test_utils.runSinglePeerTransfer(grt, .aes_256_gcm, 20 * 1024 * 1024, grt.std.math.maxInt(u64));
        }

        fn plaintext(_: *testing_api.T, allocator: grt.std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_test_utils.runSinglePeerTransfer(grt, .plaintext, 20 * 1024 * 1024, grt.std.math.maxInt(u64));
        }
    };
