...
pub fn make(comptime std: type) testing_api.TestRunner {
    const Cases = struct {
        fn chacha(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runMultiPeerBidirectionalRekey(std, .chacha_poly, 50, 50 * 1024, 32);
        }

        fn aes(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runMultiPeerBidirectionalRekey(std, .aes_256_gcm, 50, 50 * 1024, 32);
        }

        fn plaintext(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runMultiPeerBidirectionalRekey(std, .plaintext, 50, 50 * 1024, 32);
        }
    };
