...
pub fn make(comptime lib: type) testing_api.TestRunner {
    const Cases = struct {
        fn chacha(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runMultiPeerBidirectionalRekey(lib, .chacha_poly, 50, 50 * 1024, 32);
        }

        fn aes(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runMultiPeerBidirectionalRekey(lib, .aes_256_gcm, 50, 50 * 1024, 32);
        }

        fn plaintext(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runMultiPeerBidirectionalRekey(lib, .plaintext, 50, 50 * 1024, 32);
        }
    };
