...
pub fn make(comptime lib: type) testing_api.TestRunner {
    const Cases = struct {
        fn chacha(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runSinglePeerTransfer(lib, .chacha_poly, 1024 * 1024, std.math.maxInt(u64));
        }

        fn aes(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runSinglePeerTransfer(lib, .aes_256_gcm, 1024 * 1024, std.math.maxInt(u64));
        }

        fn plaintext(_: *testing_api.T, allocator: std.mem.Allocator) !void {
            _ = allocator;
            _ = try noise_testutils.runSinglePeerTransfer(lib, .plaintext, 1024 * 1024, std.math.maxInt(u64));
        }
    };
