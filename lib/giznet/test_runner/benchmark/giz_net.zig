    ...
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: grt.std.mem.Allocator) bool {
            ...
            runCase(grt, allocator, .chacha_poly) catch |err| {
                t.logErrorf("benchmark/giznet/giz_net chacha_poly concurrent_transfer failed: {}", .{err});
                return false;
            };
            runCase(grt, allocator, .aes_256_gcm) catch |err| {
                t.logErrorf("benchmark/giznet/giz_net aes_256_gcm concurrent_transfer failed: {}", .{err});
                return false;
            };
            runCase(grt, allocator, .plaintext) catch |err| {
                t.logErrorf("benchmark/giznet/giz_net plaintext concurrent_transfer failed: {}", .{err});
                return false;
            };
            return true;
        }
...
fn benchmarkLabel(comptime cipher_kind: NoiseCipher.Kind, comptime pair_count: usize) []const u8 {
    return switch (pair_count) {
        4 => switch (cipher_kind) {
            .chacha_poly => "giznet.giz_net.udp.concurrent_transfer.chacha_poly.4_conn",
            .aes_256_gcm => "giznet.giz_net.udp.concurrent_transfer.aes_256_gcm.4_conn",
            .plaintext => "giznet.giz_net.udp.concurrent_transfer.plaintext.4_conn",
        },
        8 => switch (cipher_kind) {
            .chacha_poly => "giznet.giz_net.udp.concurrent_transfer.chacha_poly.8_conn",
            .aes_256_gcm => "giznet.giz_net.udp.concurrent_transfer.aes_256_gcm.8_conn",
            .plaintext => "giznet.giz_net.udp.concurrent_transfer.plaintext.8_conn",
        },
