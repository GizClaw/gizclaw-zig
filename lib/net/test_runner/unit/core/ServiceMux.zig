...
fn runCases(comptime lib: type, testing: anytype, allocator: mem.Allocator) !void {
    ...
    const direct_event: u8 = 0x03;
    const direct_opus: u8 = 0x10;
    ...
    try mux.input(0, direct_event, "event");
    try mux.input(2, direct_opus, "opus");
    try mux.input(0, direct_opus, "lane");

    var buf: [16]u8 = undefined;
    const event_n = try mux.readServiceProtocol(0, direct_event, &buf);
    try testing.expectEqualStrings("event", buf[0..event_n]);

    try testing.expectError(errors.Error.QueueEmpty, mux.readServiceProtocol(2, direct_opus, &buf));

    const opus_n = try mux.readServiceProtocol(0, direct_opus, &buf);
    try testing.expectEqualStrings("opus", buf[0..opus_n]);

    const default_read = try mux.read(&buf);
    try testing.expectEqual(direct_opus, default_read.protocol_byte);
    try testing.expectEqualStrings("lane", buf[0..default_read.n]);

    try mux.input(0, direct_event, "drop-me-not");
    var short_buf: [4]u8 = undefined;
    try testing.expectError(errors.Error.BufferTooSmall, mux.readServiceProtocol(0, direct_event, &short_buf));
    const retained_n = try mux.readServiceProtocol(0, direct_event, &buf);
    try testing.expectEqualStrings("drop-me-not", buf[0..retained_n]);

    try testing.expectError(errors.Error.QueueEmpty, mux.readServiceProtocol(0, direct_opus, &buf));
    try testing.expectError(errors.Error.KCPMustUseStream, mux.write(protocol.kcp, "x"));
    var reject_mux = try Package.init(allocator, noise.Key.zero, .{});
    defer reject_mux.deinit();
    try reject_mux.input(5, direct_event, "x");
    const rejected_direct = try reject_mux.read(&buf);
    try testing.expectEqual(direct_event, rejected_direct.protocol_byte);
    try testing.expectEqualStrings("x", buf[0..rejected_direct.n]);

    var no_output_mux = try Package.init(allocator, noise.Key.zero, .{
        .on_new_service = allowAllServices,
    });
    defer no_output_mux.deinit();
    try testing.expectError(errors.Error.NoSession, no_output_mux.write(direct_event, "x"));
    ...
    try tiny_mux.input(0, direct_event, "a");
    try testing.expectError(errors.Error.InboundQueueFull, tiny_mux.input(0, direct_opus, "b"));
    ...
    frame[frame_n + 1] = 0;
    try testing.expect(ServiceMuxFile.shouldRejectStoppedService(protocol.kcp, frame[0 .. frame_n + 2]));
    try mux.stopAcceptingService(9);
    try testing.expectError(errors.Error.ServiceRejected, mux.input(9, protocol.kcp, frame[0 .. frame_n + 2]));
    try testing.expectEqual(@as(u64, 9), output_capture.service);
    try testing.expectEqual(protocol.kcp, output_capture.protocol_byte);
