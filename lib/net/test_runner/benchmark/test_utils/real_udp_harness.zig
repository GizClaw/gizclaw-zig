...
pub fn make(comptime lib: type) type {
    ...
    return struct {
        ...
        const SwitchablePacketConn = struct {
            ...
            pub fn flush(self: *SwitchablePacketConn) PacketConn.WriteToError!bool {
        ...
        pub fn flushClientWrites(self: *Self) !void {
            _ = try self.flushClientWritesChanged();
        }

        pub fn flushServerWrites(self: *Self) !void {
            _ = try self.flushServerWritesChanged();
        }

        pub fn pumpServer(self: *Self) !void {
            _ = try self.pumpServerNonBlocking();
        }

        pub fn pumpClient(self: *Self) !void {
            _ = try self.pumpClientNonBlocking();
        }
        ...
        fn tickBoth(self: *Self) !void {
            const now_ms = self.nextClockMs();
            try self.client_udp.testTickAt(now_ms);
            try self.server_udp.testTickAt(now_ms);
        }

        fn flushClientWritesChanged(self: *Self) !bool {
            return try self.client_wrapper.flush();
        }

        fn flushServerWritesChanged(self: *Self) !bool {
            return try self.server_wrapper.flush();
        }

        fn pumpServerNonBlocking(self: *Self) !bool {
            return try self.pumpUdpNonBlocking(self.server_wrapper, &self.server_udp);
        }

        fn pumpClientNonBlocking(self: *Self) !bool {
            return try self.pumpUdpNonBlocking(self.client_wrapper, &self.client_udp);
        }

        fn pumpUdpNonBlocking(self: *Self, wrapper: *SwitchablePacketConn, udp: *UdpType) !bool {
            const original_timeout = wrapper.read_timeout_ms;
            wrapper.setReadTimeout(0);
            defer wrapper.setReadTimeout(original_timeout);

            var progressed = false;
            var packet_count: usize = 0;
            while (packet_count < max_packets_per_pump) : (packet_count += 1) {
                const recv = wrapper.readFrom(udp.read_buf) catch |err| switch (err) {
                    error.TimedOut => break,
                    else => return err,
                };
                progressed = true;
                _ = try udp.testHandleDatagram(
                    udp.read_buf[0..recv.bytes_read],
                    @ptrCast(&recv.addr),
                    recv.addr_len,
                    self.nextClockMs(),
                );
        ...
        fn syncClock(self: *Self) void {
            const wall_now = nowMs();
            if (wall_now > self.clock_ms) self.clock_ms = wall_now;
        }

        fn nextClockMs(self: *Self) u64 {
            const wall_now = nowMs();
            const next = self.clock_ms + 1;
            self.clock_ms = if (wall_now > next) wall_now else next;
