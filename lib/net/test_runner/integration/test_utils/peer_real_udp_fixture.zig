...
// level to peer dial/accept/service semantics.
pub fn make(comptime lib: type) type {
    ...
    const BaseFixture = RealUdpFixtureFile.make(lib);
    ...
    return struct {
        ...
        pub fn init(allocator: dep.embed.mem.Allocator, options: Options) !Self {
            const base = try allocator.create(BaseFixture);
            errdefer allocator.destroy(base);
            base.* = try BaseFixture.init(allocator, .{
        ...
        pub fn dialAndAccept(self: *Self) !void {
            ...
            var ctx = try self.base.ctx_api.withTimeout(self.base.ctx_api.background(), BaseFixture.default_timeout_ns);
            defer ctx.deinit();
        ...
        pub fn reconnectClient(self: *Self) !*Peer.Conn {
            // Reconnect dedupe needs the server core accept path to advance the
            // underlying transport without manufacturing a second peer.Listener
