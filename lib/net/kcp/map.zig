...
fn bucketForInt(capacity: usize, key: anytype) usize {
    return @as(usize, @intCast(mix64(@as(u64, @intCast(key))) & @as(u64, @intCast(capacity - 1))));
}
...
pub fn testAll(testing: anytype, allocator: mem.Allocator) !void {
    var map_value = try UIntMap(u64, u64).init(allocator, 1);
