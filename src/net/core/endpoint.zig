...
pub const Endpoint = struct {
    ...
    pub fn parse(s: []const u8) ?Endpoint {
        ...
        if (!mem.eql(u8, host, "0.0.0.0")) {
            ...
            var idx: usize = 0;
            while (octets_it.next()) |octet_str| {
    ...
    pub fn eql(self: Endpoint, other: Endpoint) bool {
        return self.port == other.port and mem.eql(u8, &self.addr, &other.addr);
    }
};
...
test "Endpoint.eql" {
    const a = Endpoint.init(.{ 10, 0, 0, 1 }, 5000);
    const b = Endpoint.init(.{ 10, 0, 0, 1 }, 5000);
    const c = Endpoint.init(.{ 10, 0, 0, 2 }, 5000);
