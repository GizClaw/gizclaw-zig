...
pub fn make(comptime grt: type, comptime ZuxAppType: type) type {
    ...
    return struct {
        ...
        pub fn render(self: *Self, app: *ZuxAppType.ImplType) !void {
            ...
            try self.syncIntent(stores.app_state.get(), stores.wifi_state.get());
        }
        ...
        fn syncIntent(self: *Self, app_state: AppState, wifi_state: WifiState) !void {
            if (app_state.state == .on and wifi_state.state == .on) {
                try self.zux_app.impl.set_wifi_sta_power_save(.wifi, .none);
