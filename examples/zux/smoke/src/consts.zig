...
pub const gizclaw_speed_test_content_length: i64 = 1 * 1024 * 1024;
...
pub fn parseGizclaw(comptime sdk: type, comptime Client: type) !GizclawConfig {
    const server_key = try sdk.key.parse(gizclaw_server_key);
    return .{
        .server_key = server_key,
        .key_pair = try sdk.key.fromPrivate(try sdk.key.parse(gizclaw_client_key)),
