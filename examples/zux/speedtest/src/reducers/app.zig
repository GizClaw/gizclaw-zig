...
pub const AutoStartEvent = struct {
    pub const event_name = "smoke.auto_start";
    ...
    pub fn decodeJson(_: glib.std.mem.Allocator, _: glib.std.json.Value) !*@This() {
        return error.UnsupportedSmokeAutoStartJson;
    }
...
pub fn make(comptime grt: type, comptime ZuxAppType: type) type {
    ...
    return struct {
        ...
        pub fn reduce(
            ...
            switch (message.body) {
                .custom => |custom| {
                    ...
                    if (custom.as(AutoStartEvent)) |_| {
                        setAutoStart(stores);
                        log.info("app auto start state=on wifi=on", .{});
                        return;
                    } else |_| {}
        ...
        fn setAutoStart(stores: *Stores) void {
            stores.app_state.invoke(@as(AppStateValue, .on), struct {
                fn apply(app: *AppState, value: AppStateValue) void {
                    app.state = value;
                }
            }.apply);
            stores.wifi_state.invoke(@as(WifiStateValue, .on), struct {
                fn apply(wifi: *WifiState, value: WifiStateValue) void {
                    wifi.state = value;
                }
            }.apply);
        }
